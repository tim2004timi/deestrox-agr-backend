...
class Chat(Base):
    ...
    messager = Column(String(16), nullable=False, default="telegram") # telegram, whatsapp, instagram, vk
...
async def create_chat(db: AsyncSession, uuid: str, ai: bool = True, name: str = "Не известно", tags: List[str] = None, messager: str = "telegram", archived: bool = False, blocked: bool = False, status: str = "ongoing", is_awaiting_manager_confirmation: bool = False):
    new_chat = Chat(
        uuid=uuid,
        ai=ai,
        name=name,
        ...
        blocked=blocked,
        status=status,
        is_awaiting_manager_confirmation=is_awaiting_manager_confirmation
    )
    db.add(new_chat)
    try:
        await db.commit()
...
async def update_chat_archived(db: AsyncSession, chat_id: int, archived: bool):
    chat = await get_chat(db, chat_id)
    if chat:
...
async def update_chat_blocked(db: AsyncSession, chat_id: int, blocked: bool):
    chat = await get_chat(db, chat_id)
    if chat:
...
async def update_chat_manager_confirmation(db: AsyncSession, chat_id: int, is_awaiting_manager_confirmation: bool):
    chat = await get_chat(db, chat_id)
    if chat:
        chat.is_awaiting_manager_confirmation = is_awaiting_manager_confirmation
        await db.commit()
        await db.refresh(chat)
    return chat
...
async def get_stats(db: AsyncSession):
    ...
    archived_count = await db.scalar(select(func.count(Chat.id)).filter(Chat.archived == True))
    blocked_count = await db.scalar(select(func.count(Chat.id)).filter(Chat.blocked == True))
    ongoing_count = await db.scalar(select(func.count(Chat.id)).filter(Chat.status == "ongoing"))
    closed_count = await db.scalar(select(func.count(Chat.id)).filter(Chat.status == "closed"))
    return {
        "total": total, 
        "pending": pending, 
        "ai": ai_count,
