...
from crud import async_session, engine, Base, get_chats, get_chat, get_messages, create_chat, create_message, update_chat_waiting, update_chat_ai, update_chat_archived, update_chat_blocked, update_chat_status, update_chat_manager_confirmation, get_stats, get_chats_with_last_messages, get_chat_messages, get_chat_by_uuid, add_chat_tag, remove_chat_tag
import requests
...
class ArchivedUpdate(BaseModel):
    archived: bool

@app.put("/api/chats/{chat_id}/archived")
async def update_archived(chat_id: int, data: ArchivedUpdate, db: AsyncSession = Depends(get_db)):
    chat = await crud.update_chat_archived(db, chat_id, data.archived)
    if not chat:
        raise HTTPException(status_code=404, detail="Chat not found")
    # Отправляем обновление по WebSocket
    update_message = {
        "type": "chat_archived_updated",
        "chatId": str(chat_id),
...
@app.put("/api/chats/{chat_id}/blocked")
async def update_blocked(chat_id: int, data: BlockedUpdate, db: AsyncSession = Depends(get_db)):
    chat = await crud.update_chat_blocked(db, chat_id, data.blocked)
    if not chat:
        raise HTTPException(status_code=404, detail="Chat not found")
    # Отправляем обновление по WebSocket
    update_message = {
        "type": "chat_blocked_updated",
        "chatId": str(chat_id),
...
@app.put("/api/chats/{chat_id}/status")
async def update_status(chat_id: int, data: StatusUpdate, db: AsyncSession = Depends(get_db)):
    chat = await crud.update_chat_status(db, chat_id, data.status)
    if not chat:
        raise HTTPException(status_code=404, detail="Chat not found")
    # Отправляем обновление по WebSocket
    update_message = {
...
class ManagerConfirmationUpdate(BaseModel):
    is_awaiting_manager_confirmation: bool

@app.put("/api/chats/{chat_id}/manager_confirmation")
async def update_manager_confirmation(chat_id: int, data: ManagerConfirmationUpdate, db: AsyncSession = Depends(get_db)):
    chat = await crud.update_chat_manager_confirmation(db, chat_id, data.is_awaiting_manager_confirmation)
    if not chat:
        raise HTTPException(status_code=404, detail="Chat not found")
    # Отправляем обновление по WebSocket
    update_message = {
        "type": "chat_manager_confirmation_updated",
        "chatId": str(chat_id),
